...
public class Class {
	...
	public static int Occurences(int[][] tempboard,int Dimension,int type) {
		//type 1 means we have to check for mine cells, type 2 means we have to check for safe
		//the method returns total number of type cells identified in the board
		int count=0;
		if(type==1) {
			for(int i=0;i<Dimension;i++) {
				for(int j=0;j<Dimension;j++) {
			...
			for(int i=0;i<Dimension;i++) {
				for(int j=0;j<Dimension;j++) {
					if(tempboard[i][j]==-2 || tempboard[i][j]==0) {
						count+=1;
					}
				}
			}
	...
	public static cell ReturnUnidentifiedCell(int[][] tempboard,cell original,int Dimension) {
		int row = original.row;
		int col = original.col;
		cell ptr = new cell();
		ptr.row=-1;
		ptr.col=-1;
		...
		if(row-1>=0 && col+1<Dimension && tempboard[row-1][col+1]==-1) {
			ptr.row=row-1;
			ptr.col=col+1;
			return ptr;
		}
		...
		if(col+1<Dimension && tempboard[row][col+1]==-1) {
			ptr.row=row;
			ptr.col=col+1;
			return ptr;
		}
		if(col-1>=0 && row+1<Dimension && tempboard[row+1][col-1]==-1) {
			ptr.row=row+1;
			ptr.col=col-1;
			return ptr;
		}
	...
	public static cell betterCellSelection(int[][] array,int[][] tempboard,int Dimension) {
		cell temp = new cell(); //this cell will be returned as the next cell
		temp.row = -1;
		temp.col = -1;
		cell ptr = new cell(); //this cell will be used in interation of cells
		ptr.row = -1;
		ptr.row = -1;
		cell ref = new cell(); //this cell will be used to return the unidentified cell from another method 
		ref.row = -1;
		ref.col = -1;
		double prevProb = 1;
		int minesIdentified;
		...
		for(int i=0;i<Dimension;i++) {
			for(int j=0;j<Dimension;j++) {
				if(tempboard[i][j]>=1) { //checking a safe cell
					ptr.row = i;
					ptr.col = j;
					minesIdentified = identifyMines(tempboard,Dimension,ptr);
					hiddencells = hiddenNeighbors(tempboard,Dimension,ptr);
					if(hiddencells!=0) { //if there are no hidden neighbors then we move to next cell
						if((double)((array[i][j]-minesIdentified)/hiddencells)<prevProb) { //checking the probability of neighbors having a mine
							ref = ReturnUnidentifiedCell(tempboard,ptr,Dimension); //checking if a hidden cell neighbor exists for this cell
							if(ref.row!=-1) {
								//update the new lowest probability cell
								prevProb = (double)((array[i][j]-minesIdentified)/hiddencells);
								temp=ref;
							}
						}
					}	
	...
	public static cell nextCell(int[][] tempboard,int Dimension) {
		//Randomly selecting an unidentified cell
		cell temp=new cell();
		temp.row=-1;
		temp.col=-1;
		Random rand = new Random();
	...
	public static int basicAlgo(int[][] array, int Dimension,int mines) {
		...
		int totalSafe = (Dimension*Dimension)-mines;
		...
		while(true) { 
			/*if(totalSafe==Occurences(tempboard,Dimension,1) || mines==Occurences(tempboard,Dimension,2)) { 
				//If we have identified all mines or all safe cells before completing the board, then we know all the other cells 
				return (mines-mineCount);
			}
			*/ //This is for global information
			...
			if(ptr.row!=-1 && ptr.col!=-1) {  //true if there is an unexplored safe cell so we will explore this cell
				...
				ptr=betterCellSelection(array,tempboard,Dimension); //this can also return -1 if there are no safe cells yet identified
																	//so in this case we use the old method of randomly returning
				if(ptr.row==-1) {
					ptr=nextCell(tempboard,Dimension);
				}
				//if this method also returns -1 then there all cells have been identified
	...
	public static int ImprovedAlgo(int[][] array, int Dimension, int mines) {
		...
		int mineCount=0;
		int totalSafe = (Dimension*Dimension)-mines; //total safe cells in minesweeper
		...
		while(true) {
			...
			*/    //This is for global information
			...
			if(ptr.row!=-1) { //true if there is an unexplored safe cell
				...
				//no safe cell is left so we have to randomly pick
				ptr=betterCellSelection(array,tempboard,Dimension); //his is for the bonus question
				//if -1 is returned then there are no safe cells so we use old method
				if(ptr.row==-1) {
					ptr=nextCell(tempboard,Dimension);
				}
				if(ptr.row==-1 && ptr.col==-1) { //this means that all cells are completed
	...
	public static void main(String[] args) {
		//There are some extra arrays and variables which I used for displaying the results for my own convenience.
		//I had to use them for the plots and checking them one by was hectic
		//These extra arrays are dens,baseCheck and improveCheck
		long startTime = System.currentTimeMillis();
		DecimalFormat df2 = new DecimalFormat("#.##");
		int mines=200; //starting mines are 200
		int base=0;
		int improve=0;
		double density=0;
		int[][] check;
		...
		double[] baseCheck = new double[50];
		double[] improveCheck = new double[50];
		int c=0;
		while(mines<9900) { 
			for(int i=0; i<10; i++) { //iterating the algorithms 10 times to get a good score at a given mine density
				...
				density=(double)mines/10000; //10000 are the total cells
			...
			dens[c] = density;
			baseCheck[c]=(double)base/11/mines;
			improveCheck[c]=(double)improve/11/mines;
			...
			System.out.println();
			mines=mines+200; //incrementing mines by 200
			//resetting base and improve values for the next mine density
			base=0; 
			improve=0;
			c+=1; //this is for the extra arrays
		}
		for(int i=0;i<50;i++) {
			System.out.print(df2.format(dens[i]) + ",");
		}
		for(int i=0;i<50;i++) {
			System.out.print(df2.format(baseCheck[i]) + ",");
		}
		System.out.println();

		for(int i=0;i<50;i++) {
			System.out.print(df2.format(improveCheck[i]) + ",");
		}
		System.out.println();
        runGC();
